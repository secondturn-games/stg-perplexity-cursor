 ...
 * ```
 */
export class MemoryCacheRepository<T = any>
  implements IMemoryCacheRepository<T>
{
  ...
  async getMany<K = T>(keys: string[]): Promise<Record<string, K>> {
    try {
      ...
      throw new Error(
        `Cache getMany error: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async setMany<K = T>(
    ...
    try {
      ...
      throw new Error(
        `Cache setMany error: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async getWithMetadata<K = T>(
    ...
    try {
      ...
      return {
        value: entry.value as unknown as K,
      ...
      throw new Error(
        `Cache getWithMetadata error for key ${key}: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async extendTtl(key: string, ttl: number): Promise<boolean> {
    try {
      ...
      throw new Error(
        `Cache extendTtl error for key ${key}: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async getMemoryInfo(): Promise<{
    ...
    try {
      ...
      throw new Error(
        `Memory info error: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async forceGC(): Promise<void> {
    try {
      if (isGCAvailable()) {
        ...
        console.warn(
          'Garbage collection is not available. Run Node.js with --expose-gc flag.'
        );
      }
    } catch (error) {
      throw new Error(
        `Force GC error: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
   ...
   * ```
   */
  async getEntriesWithMetadata<K = T>(
    pattern: string
  ): Promise<
    Array<{
      key: string;
  ...
  > {
    try {
      ...
      throw new Error(
        `Get entries with metadata error for pattern ${pattern}: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private updateHitRate(): void {
    if (this.stats.totalQueries > 0) {
      this.stats.hitRate =
        Math.round(
          (this.stats.totalHits / this.stats.totalQueries) * 100 * 100
        ) / 100;
    }
  }
...
// Type guard for gc function
function isGCAvailable(): boolean {
  return (
    typeof global !== 'undefined' && typeof (global as any).gc === 'function'
  );
}
