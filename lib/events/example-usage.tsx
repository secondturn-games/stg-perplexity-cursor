...
export function EventSystemExample() {
  ...
  useBGGEventHandler((event: BGGEvent) => {
    ...
    setStats(prev => ({
      ...
      searchEvents: event.eventType.includes('search')
        ? prev.searchEvents + 1
        : prev.searchEvents,
      cacheEvents: event.eventType.includes('cache')
        ? prev.cacheEvents + 1
        : prev.cacheEvents,
      errorEvents:
        ...
        <div className='max-h-96 overflow-y-auto'>
          {events.length === 0 ? (
            <p className='text-gray-500 text-center py-4'>
              No events yet. Click a button above to emit events.
            </p>
          ) : (
  ...
  useBGGEventHandler((event: BGGEvent) => {
    if (event.eventType === 'game.searched') {
      setSearchStats(prev => ({
        ...
        averageQueryTime:
          (prev.averageQueryTime + event.data.performance.queryTime) / 2,
      ...
      <div className='grid grid-cols-3 gap-4'>
        ...
        <div className='text-center'>
          <div className='text-xl font-bold'>
            {Math.round(searchStats.averageQueryTime)}ms
          </div>
          <div className='text-sm text-gray-600'>Avg Query Time</div>
        </div>
        <div className='text-center'>
          <div className='text-xl font-bold'>
            {Math.round(searchStats.cacheHitRate * 100)}%
          </div>
        ...
        <div className='space-y-2'>
          {errors
            ...
            .map((error, index) => (
              <div
                key={index}
                className='bg-red-50 border-l-4 border-red-500 p-2'
              >
